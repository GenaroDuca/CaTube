...
// =================================================================
// Máximo 255 caracteres. Permitir letras, números, espacios y caracteres comunes de puntuación.
// Nota: La expresión regex es más flexible aquí para una descripción, pero la limitación de 255 caracteres se maneja en el handler.
// Permitimos casi cualquier carácter para una descripción, excepto quizás saltos de línea y caracteres de control.
const CHANNEL_DESCRIPTION_REGEX = /^[\s\S]{0,255}$/; // Permite cualquier caracter (incluyendo saltos de línea) hasta 255.
...
function InfoContainer({ channelId }) {
        ...
        async function loadChannelData() {
            try {
                ...
                if (channelData) {
                    ...
                    // === CAMBIO CLAVE: Nueva validación para descripción ===
                    setDescriptionValid(initialDescription.length <= 255);
                    // =======================================================
                }
            } catch (error) {
    ...
    // === CAMBIO CLAVE: Limitación de 255 caracteres y Validación ===
    const handleDescriptionChange = (e) => {
        const value = e.target.value;
        if (value.length <= 255) { // Controla la longitud del input
            setDescription(value);
        }
        // Valida si la longitud actual (después de la posible limitación) es 255 o menos.
        setDescriptionValid(value.length <= 255);
    ...
    const handlePublish = async () => {
        if (!channelId) {
            alert(
                "Error: No se pudo identificar el canal. Por favor, inicia sesión de nuevo."
            );
            return;
        }
        ...
        const isNameValid = name.length > 0 ? CHANNEL_NAME_REGEX.test(name) : true;
        const isHandleValid =
            handle.length > 0 ? CHANNEL_NAME_REGEX.test(handle) : true;

        // === CAMBIO CLAVE: Nueva validación de 255 caracteres para la descripción ===
        // Si no está vacío, debe tener <= 255. Si está vacío, es válido (true).
        const isDescriptionValid =
            description.length > 0 ? description.length <= 255 : true;
        // ===========================================================================
        ...
        try {
            ...
            if (result) {
                ...
                const errorMessage =
                    (result && result.message) || "Ocurrió un error desconocido.";
    ...
    const getInputClass = (fieldName) => {
        ...
        if (!isTouched || value === "")
            return fieldName === "description"
                ? "rectangle-customization-description"
